...
function ExperienceSection() {
  ...
  return (
    <div className={`flex flex-col gap-5 p-5 w-full h-full ${darkMode ? 'bg-zinc-700' : 'bg-zinc-100'} rounded-md`}>
      <div className="flex flex-col items-center">
        <div className="flex flex-wrap justify-evenly gap-4 mt-3">
          {
            [...certificates].reverse().map((certificate, index) => {
              return (
                <div className={`${darkMode ? 'bg-gray-600' : 'bg-white'} p-3 rounded-md`} key={index}>
                  <img 
